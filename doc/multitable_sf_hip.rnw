...
library("knitr")
library("ggplot2")
opts_chunk$set(cache = TRUE, fig.width = 7, fig.height = 5,
...
Our results are shown in figures.. The scores had already been smooth
on the maps from our earlier approaches, even without enforcing
spatial homogeneity, but this quality is even more pronounced
here. That said, many of the overall relationships across features and
between features and census tracts do seem consistent across methods.
