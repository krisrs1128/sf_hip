...
opts_chunk$set(cache = TRUE, fig.width = 8, fig.height = 8,
               fig.align = "center", message = F, warning = F)
theme_set(theme_bw())

sapply(list.files("../src", full.names = TRUE), read_chunk)
@

\linespread{1.5}

\title{A Case Study in Multitable Data Analysis The SFHIP Data}
...
for subsequent methods. See Figures \ref{fig:pca_map_1},
\ref{fig:pca_map_2}, \ref{fig:pca_scores}, \ref{fig:pca_loadings} for
the results.
...
@
\label{fig:pca_map_1}
\caption{This shades each census tract in San Francisco by the
  first PCA component.}
\end{figure}
...
work with pairs of tables. There are only three pairs here, though,
and it is not too overwhelming to view the scores and loadings for
each combination.
