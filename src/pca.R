...
## ---- pca-scores ----
ggplot(tract_scores) +
  geom_text(aes(x = Comp.1, y = Comp.2, label = Tract2010), size = 2) +
  ggtitle("PCA Scores")
...
ggplot(tract_loadings) +
  geom_text(aes(x = Comp.1, y = Comp.2, label = variable, col = group), size = 2) +
...
ggplot(tract_scores_map) +
  ...
  facet_wrap(~ variable) +
  coord_fixed() +
  ggtitle("PCA Score across Census Tracts")
